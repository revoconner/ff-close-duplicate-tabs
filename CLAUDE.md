...
- Clicking the icon opens a small popup that shows the result of the run it just triggered:
    ...
    - Below it a list, one row per URL that had duplicates, with the URL and the number of tabs closed for it.
...
- Planned files: `manifest.json`, `background.js`, `popup.html`, `popup.js`, `options.html`, `options.js`, `style.css` (shared dark theme), `icon.png`. No build step, no dependencies.
